...
# We also report that we directly depend on $(1). Technically this isn't
# needed, since the .ec dependency information also depends on $(1), but
# providing this information makes it easier for tools other than make
# to notice the dependency (because we make it a direct dependency).
# We also add a dependency on "scanned..." unless SKIP_SCANS is defined;
# we don't have to do this, but doing this means that every file
# must pass a scan before we run it
# (this encourages people to fix scan problems ASAP).
uses = deps/$(strip $(1)).ec $(1) \
       $(if $(SKIP_SCANS),,scanned/$(strip $(1)).scan)
